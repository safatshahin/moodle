...
class matrix_form_definition extends communication_form_base {
    ...
    public static function save_form_data(\stdClass $instance, int $communicationid): void {
        if (!empty($communicationid)) {
            $matrixroomtopic = $instance->matrixroomtopic ?? '';
            $matrixroomdata = new matrix_rooms($communicationid);
            $matrixroomdata->topic = $matrixroomtopic;
            if ($matrixroomdata->record_exist()) {
                $matrixroomdata->update();
            } else {
                $matrixroomdata->create();
            }
        }
    }
