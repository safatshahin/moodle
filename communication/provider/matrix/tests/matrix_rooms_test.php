...
class matrix_rooms_test extends \advanced_testcase {
     ...
     * @return void
     * @covers ::create
     * @covers ::get_room_alias
     * @covers ::get_roomid
     * @covers ::get_matrix_room_data
     * @covers ::set_roomid
     * @covers ::set_room_alias
     */
    public function test_matrix_room_record_creation(): void {
        ...
        // Communication handler call.
        $communication = new communication_handler($course);

        // Call matrix room object to create the matrix data.
        $matrixroom = new \communication_matrix\matrix_rooms($communication->communicationsettings->get_communication_instance_id());
        $matrixroom->set_roomid($sampleroomid);
        $matrixroom->set_room_alias($sampleroomalias);
        $matrixroom->create();

        // Test the object.
        $this->assertEquals($matrixroom->get_roomid(), $sampleroomid);
        $this->assertEquals($matrixroom->get_room_alias(), $sampleroomalias);

        // Get the record from db.
        $matrixrecord = $DB->get_record('matrix_rooms',
            ['commid' => $communication->communicationsettings->get_communication_instance_id()]);
        ...
        $this->assertEquals($communication->communicationsettings->get_communication_instance_id(), $matrixrecord->commid);
    }
    ...
    public function test_matrix_room_record_update(): void {
        ...
        $matrixroom->set_roomid($sampleroomidupdated);
        $matrixroom->set_room_alias($sampleroomaliasupdated);
        $matrixroom->update();

        // Test the object.
        $this->assertEquals($matrixroom->get_roomid(), $sampleroomidupdated);
        $this->assertEquals($matrixroom->get_room_alias(), $sampleroomaliasupdated);
