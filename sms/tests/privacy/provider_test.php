...
final class provider_test extends \core_privacy\tests\provider_testcase {
    ...
    public function test_delete_data_for_users(): void {
        ...
        // Data is all stored under the users own contect.
        // The passed context is ignored and the actual user context is used.
        provider::delete_data_for_users(new approved_userlist(
            $usercontext,
            'core_sms',
            [$otheruser->id]
        ));
        $this->assertEmpty($DB->get_records('sms_messages', ['recipientuserid' => $otheruser->id]));
        $this->assertEquals(2, $DB->count_records('sms_messages'));

        // Deleting data for both users only delets that user's content.
        provider::delete_data_for_users(new approved_userlist(
            $usercontext,
            'core_sms',
        ...
        $this->assertEmpty($DB->get_records('sms_messages', ['recipientuserid' => $user->id]));
        $this->assertEquals(0, $DB->count_records('sms_messages'));
    }
