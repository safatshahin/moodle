...
/**
 * SMS gateway plugins manager. Allow to enable/disable sms gateway plugins and jump to settings.
 *
 * @package    core_sms
 * @copyright  2024 Safat Shahin <safat.shahin@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class manage_smsgateway_page extends admin_setting {
    ...
    public function output_html($data, $query = ''): string {
        ...
        $plugins = $pluginmanager->get_plugins_of_type('smsgateway');
        if (empty($plugins)) {
            return get_string('nocsmsgateway', 'core_sms');
        }
        ...
        $table->attributes['class'] = 'managesmsgatwwaytable generaltable admintable';
        $table->data = [];

        foreach ($plugins as $plugin) {
            $class = '';
            $actionurl = new moodle_url('/admin/smsgateway.php', ['sesskey' => sesskey(), 'name' => $plugin->name]);

            if (
                $pluginmanager->get_plugin_info('smsgateway_' . $plugin->name)->get_status() ===
            ...
            if ($uninstallurl = core_plugin_manager::instance()->get_uninstall_url(
                'smsgateway_' . $plugin->name, 'manage')) {
    ...
    public function is_related($query): bool {
        ...
        $types = core_plugin_manager::instance()->get_plugins_of_type('smsgateway');
        foreach ($types as $type) {
            if (
                str_contains($type->component, $query) ||
