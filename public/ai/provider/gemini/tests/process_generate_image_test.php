...
use aiprovider_gemini\test\testcase_helper_trait;
...
/**
 * Test response_base gemini provider methods.
 *
...
final class process_generate_image_test extends \advanced_testcase {
    ...
    public function test_calculate_size(): void {
        ...
        $this->assertEquals('1:1', $size);

        $ratio = 'portrait';
        $size = $method->invoke($processor, $ratio);
        $this->assertEquals('9:16', $size);

        $ratio = 'landscape';
        $size = $method->invoke($processor, $ratio);
        $this->assertEquals('16:9', $size);
    }
    ...
    public function test_handle_api_success(): void {
        ...
        $this->assertNotNull($result['imagebase64']);
    }
    ...
    public function test_prepare_response_success(): void {
        ...
        $this->assertNotNull($response['imagebase64']);
    }
    ...
    public function test_process(): void {
        ...
        $this->assertTrue($result->get_success());
        $this->assertEquals('generate_image', $result->get_actionname());
        $this->assertNotNull($result->get_response_data()['imagebase64']);
    }
