...
/**
 * Tests for the overflow ("More" menu) behaviour of the shared secondary/primary navigation.
 *
...
import SecondaryNav, {type SecondaryNavNode} from '@moodle/lms/core/nav/SecondaryNav';
 ...
 * @returns The nodes.
 */
const makeItems = (labels: string[]): SecondaryNavNode[] => labels.map((text) => ({
 ...
 * @returns The node.
 */
const makeSubmenuItem = (text: string, childlabels: string[]): SecondaryNavNode => ({
    ...makeItems([text])[0],
 ...
 */
const renderItems = (items: SecondaryNavNode[], capacity: number, istablist = false): HTMLElement => {
    rowCapacity = capacity;
    return render(
        <SecondaryNav items={items} morelabel="More" istablist={istablist} />,
    ).container;
};
...
describe('@moodle/lms/core/nav/SecondaryNav overflow', () => {
...
// A custom menu parent item and a secondary navigation branch node both arrive with
...
describe('@moodle/lms/core/nav/SecondaryNav submenus in the More menu', () => {
    const SUBMENU_ITEMS: SecondaryNavNode[] = [
