 ...
 * @coversDefaultClass \core_course\hook\after_course_updated
 */
class after_course_updated_test extends \advanced_testcase {
    ...
    public function test_get_hook_description(): void {
        $this->assertIsString(
            actual: after_course_updated::get_hook_description(),
        );
    }
    ...
    public function test_hook_tags(): void {
        $this->assertIsArray(
            actual: after_course_updated::get_hook_tags(),
        );
        $this->assertSame(
            expected: ['course'],
            actual: after_course_updated::get_hook_tags(),
    ...
    public function test_hook_dispatch(): void {
        ...
        $count = 0;
        $receivedhook = null;
        $testcallback = function(after_course_updated $hook) use (&$receivedhook, &$count): void {
            $count++;
            $receivedhook = $hook;
        };

        $this->redirectHook(
            hookname: after_course_updated::class,
        ...
        $this->assertInstanceOf(
            expected: after_course_updated::class,
            actual: $receivedhook,
        );
        $this->assertSame(
            expected: $course1->id,
