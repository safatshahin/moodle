...
if (count($gatewayrecords) > 0) {
    ...
    $settings->add(
        new admin_setting_configselect(
            ...
            get_string('settings:smsgateway', 'factor_sms'),
            get_string('settings:smsgateway_help', 'factor_sms', $smsconfigureurl),
            '',
            $gateways,
        ),
    );
    ...
    if (get_config('factor_sms', 'enabled')) {
        $settings->add(
            new admin_setting_configtext('factor_sms/weight',
        ...
        $settings->add(
            new admin_setting_configduration('factor_sms/duration',
            get_string('settings:duration', 'tool_mfa'),
            get_string('settings:duration_help', 'tool_mfa'),
                30 * MINSECS,
                MINSECS,
            ),
        );
